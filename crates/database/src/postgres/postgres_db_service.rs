...
const BLOCK_SUBMISSION_FIELD_COUNT: usize = 21;
...
impl PostgresDatabaseService {
    /// Create a new PostgresDatabaseService from a deadpool config.
    /// Primarily used for testing.
    pub fn new(config: &Config, region: i16) -> Result<Self, deadpool_postgres::CreatePoolError> {
        let pool = config.create_pool(None, NoTls)?;
        let high_priority_pool = config.create_pool(None, NoTls)?;
            ...
            loop {
                ...
                match self_clone.local_cache.pending_validator_registrations.len() {
                    ...
                    _ => {
                        ...
                        match self_clone.save_validator_registrations(&entries).await {
                            Ok(_) => {
                                ...
                                CACHE_SIZE.with_label_values(&["pending_registrations"]).set(
                                    self_clone.local_cache.pending_validator_registrations.len()
                                        as f64,
                                );
                                CACHE_SIZE
                                    .with_label_values(&["validator_registrations"])
                                    .set(self_clone.local_cache.validator_registration_cache.len()
                                        as f64);
                            }
                            Err(e) => {
    ...
    /// Save validator registrations to the database.
    /// This is an internal method used by the registration processor.
    /// Made pub(crate) for testing purposes.
    pub(crate) async fn save_validator_registrations(
        &self,
    ...
    async fn _flush_block_submissions(
        ...
        for chunk in batch.chunks(CHUNK_SIZE) {
            ...
            let mut sql = String::from(
                "INSERT INTO block_submission (block_number, slot_number, parent_hash, block_hash, builder_pubkey, proposer_pubkey, proposer_fee_recipient, gas_limit, gas_used, value, num_txs, timestamp, first_seen, region_id, optimistic_version, metadata, is_adjusted, live_ts, num_blobs, blob_gas_used, excess_blob_gas) VALUES ",
            );
...
impl PostgresDatabaseService {
    ...
    pub async fn get_validator_preferences(
        ...
        if let Some(cached_entry) = self.local_cache.validator_registration_cache.get(pub_key) {
            record.record_success();
    ...
    pub async fn update_validator_preferences(
        ...
        if let Some(mut cached_entry) =
            self.local_cache.validator_registration_cache.get_mut(pub_key)
        {
