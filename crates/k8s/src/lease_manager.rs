...
impl LeaseManager {
                ...
                Ok(acquired) => {
                    if acquired {
                        ...
                        if !self.verify_leadership().await {
                            ...
                            sleep(Duration::from_secs_f64(self.config.retry_period_secs)).await;
                            continue;
                        }
                        ...
                        if was_rotation {
                            ...
                            let backoff_secs = self.config.retry_period_secs * ROTATION_BACKOFF_MULTIPLIER;
                            info!(
                                backoff_secs,
                ...
                if let Some(spec) = &lease.spec {
                    if let Some(holder_identity) = &spec.holder_identity {
                        ...
                        if let Some(renew_time) = &spec.renew_time {
                            ...
                            if time_since_renew_secs > self.config.lease_duration_secs {
                                // Lease expired, try to take it
                                info!(
                                    current_holder = holder_identity,
                                    seconds_since_renew = time_since_renew_secs,
                                    lease_duration = self.config.lease_duration_secs,
    ...
    async fn renew_lease_loop(&self) -> Result<(), LeaseError> {
        ...
        let renew_interval = Duration::from_secs_f64(self.config.renew_deadline_secs);
                ...
                if let Some(start_slot) = start_slot_opt {
                    ...
                    if slots_held >= interval {
                        ...
                        wait_for_safe_transition(
                            ...
                            &self.auctioneer,
                            self.config.slot_completion_timeout_secs,
